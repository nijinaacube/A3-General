...
frappe.ui.form.on('Opportunity', {
    after_save: function(frm) {
        if (frm.doc.order_status == "New" && frm.doc.booking_type == "Transport") {
            ...
            var orderStatus = frm.doc.order_status;
            ...
            frappe.call({
                ...
                callback: function(r) {
                    ...
                    var vehicles = r.message.vehicles; // Retrieve vehicles data

                    // Construct the URL with the opportunity ID, order status, and vehicles as query parameters
                    ...
                    // Add vehicles to the URL
                    if (vehicles && vehicles.length > 0) {
                        var vehicleParams = '&vehicles=';
                        for (var i = 0; i < vehicles.length; i++) {
                            vehicleParams += encodeURIComponent(vehicles[i].vehicle_type) + ',';
                        }
                        vehicleParams = vehicleParams.slice(0, -1); // Remove the trailing comma
                        redirectURL += vehicleParams;
                    }
			...
			var opportunityID = frm.doc.name;
				...
				frappe.call({
					...
					callback: (r)=>{            	 
						...
						var lat = r.message.lat
						var long = r.message.long
						var vehicles = r.message.vehicles;
...
zone: function(frm, cdt, cdn) {
	...
	if (row.id10){
		...
		transit_charges_row.from = from_row.zone
		...
		if (frm.doc.vehicle_details_item){ 
		if (transit_details.length > 1) {
			const from_row = transit_details[transit_details.length - 2];
			const to_row = transit_details[transit_details.length - 1];
			// Calculate transportation cost between the current and previous zones
	...
	if (frm.doc.lead_id) {
    	...
    	if (frm.doc.booking_type !== "Diesel") {
        	...
        	frm.fields_dict['warehouse_space_details'].grid.get_field('rental_charges').get_query = function(doc, cdt, cdn) {
				...
				frm.fields_dict['transit_charges'].grid.get_field('vehicle_type').get_query = function(doc, cdt, cdn) {
					var child = locals[cdt][cdn];
					var filters = {
					
						'name': ['in', frm.doc.vehicle_details_item.map(row => row.vehicle_type)]
					};
					return {
						filters: filters
					};
...
required_area: function(frm, cdt, cdn) {
 ...
 frappe.ui.form.on('Transit Charges', {
	...
	vehicle_type: function(frm, cdt, cdn){
		...
		if (child.vehicle_type && child.from && child.to){
